...
public class Monitor extends UnicastRemoteObject implements Monitor_itf {
    /*******
     * RMI *
     *******/
    ...
    private static Monitor_itf monMon;		    // référence distante du moniteur
    ...
    public static void init() {
        try {
            Monitor_itf monMon = new Monitor();
    ...
    public void greenLight(String site, int facteur) throws RemoteException {
    ...
    public void signal(String événement,String site,int idReg) throws RemoteException {
        String registre = suivisInv.get(idReg);
        if (suivis.contains(registre)) {
            System.out.println(site + " : " + événement);
            listRegistry(registre);
        }
    }


    public static void listRegistry(String name) {
        // en-tête
    ...
    private static SharedObject getObject (String objname) {
        ...
        if (obj == null) {
            obj = (SharedObject) Client.lookup(objname);
            if (obj == null)
        ...
        actions.put ("cliche", new Action() {
            public void run (String[] args) {
                if (args[1].equals("--d")) listerDélais();
                    ...
                    try {
                        System.out.println ("============");
                        for (String nom : server.list()) listRegistry(nom);
                        System.out.println ("============");
                    } catch (RemoteException rex) {
                    ...
                    String name = extractName (args);
                    if (name != null) listRegistry(name);
                    else System.out.println ("??? (nom inconnu)");
                }
            }
        });
