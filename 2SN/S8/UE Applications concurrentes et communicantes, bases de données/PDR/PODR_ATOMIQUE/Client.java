...
public class Client extends UnicastRemoteObject implements Client_itf{
    ...
    //Number of Clients
    private static int totalNumberClients ;
    ...
    public static void init(String clientName) {
		...
		try {
            ...
            Client.totalNumberClients = Client.clients.size();
		} catch (Exception e) {
    ...
    public static void read(int idObject) {
        ...
        final Counter counter2 = new Counter(so.getVersion(),so.getObj());
        counter2.inc();
        for (Client_itf c : Client.clients) {
            ...
            new Thread(new Runnable() {
                public void run() {
                    try {
						...
						c.update(idObject, version, value);
        ...
        while (counter2.getCounter() < (minimumNumberAck)) {
            try {
                Thread.sleep(1000);
            } catch (Exception e) {
                e.printStackTrace();
