...
double dnorm2_par(double *x, int n){
  ...
  #pragma omp parallel
  {
    #pragma omp single
    {
      for(i=0; i<n; i++){
        if(x[i]!=0.0){
          #pragma omp task depend(in:x[i]) depend(out:absxi) firstprivate(i)
          absxi = fabs(x[i]);
          if(scale < absxi){
            #pragma omp task depend(inout:ssq) depend(in:absxi) depend(inout:scale)
            {
              ssq = 1.0 + ssq*pow(scale/absxi,2);
              scale = absxi;
            }
          } else {
            #pragma omp task depend(inout:ssq) depend(in:absxi) depend(in:scale) 
            ssq = ssq + pow(absxi/scale,2);
          }
        }
      }
