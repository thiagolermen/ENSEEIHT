...
struct
 ...
 let arbre4 =
  ...
  let rec get_branche e arbre = 
    match arbre with 
      | Noeud fils -> List.assoc_opt e fils
      | _ -> None
  ...
  let rec dechiffrer ls arbre = 
    let rec aux l ar =
      match l with
        | [] -> []
        | t::q -> 
          match (get_branche t ar) with
            | None -> raise CodeNonValide
            | Some a -> 
              match a with
                | Lettre a -> a::(dechiffrer q arbre)
                | Noeud fils -> aux q (Noeud fils)
    in aux ls arbre
  ...
  let chiffrer ls arbre  = fun _ -> assert false
...
struct
  ...
  type symbole_code = DRDonnee.symbole
  type code = DRDonnee.mot
