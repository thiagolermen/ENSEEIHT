...
let%test _ = empty_btree = Leaf _0
...
let rec height tr = 
    match tr with
        | Leaf _ -> 0
        | Node (g, d) -> max (1 + height g) (1 + height d)
...
let rec search tr addrl = 
    (* if List.length addrl <= height tr 
        then failwith "ERROR! Tree height is higher than the size of the address" 
    else *)
        match tr, addrl with
            | Leaf a, [] -> a
            | Node (g, d), t::q -> if t = 0 then search g q else search d q
            | _, _ -> _0
let%test "search-0" =
    search bt1 [0; 0; 0] = 'a'
let%test "search-1" =
...
let rec update tr x addrl = 
    (* if List.length addrl < height tr 
        ...
        match tr, addrl with
            | (Leaf _, []) -> Leaf x
            | (Node (g, d), t::q) -> if t = 0 then (Node ((update g x q), d)) else (Node (g, (update d x q)))
            | Leaf _0, t::q -> sprout x (t::q)
            | _, _ -> failwith "ERROR!"
