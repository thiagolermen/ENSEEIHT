...
struct
     ...
     *)
    let allocsize (bs, tr) = bs * (num_nodes tr)

    (* Occupation de l'arbre. On ne compte que les feuilles non égales à 0. *)
    let busyness (bs, tr) = 
        match tr with
            | Leaf a -> if a = _0 then 1 else 0
            | Node (g, d) -> (num_values g) + (num_values d)

    (* La mémoire vide est celle qui ne contient aucun chemin *)
    let clear bs = (bs, (sprout _0 (bits (size (bs, 0)) bs)))

    (* Implémentation de read. *)
    let read (bs, tr) addr = search tr (bits (size (bs, 0)) addr)

    (* Implémentation de write. *)
    let write (bs, tr) addr value = (bs, update tr value (bits (size (bs, 0)) addr))
end
