...
let hexadecimal = chiffre | ["a-fA-F"]
let octal = ['0'-'8']
let binary = ['0'-'1']
...
(* Nombres entiers : A COMPLETER *)
  ...
  | ("0x" ('_' | hexadecimal)*) as texte  {(ENTIER (int_of_string texte))}
  | ('0' ('0' | (['1' - '9'] ('_' |  ['0' - '7']*)))) as texte  {(ENTIER (int_of_string texte))}
  | (("0b" | "0B") ('_' |  ['0' - '1']*)) as texte  {(ENTIER (int_of_string texte))}
(* Nombres flottants : A COMPLETER *)
  | (chiffre+ "." chiffre+) as texte     { (FLOTTANT (float_of_string texte)) }
  | ("0x" ('0' | (['1' - '9'] ('_' | chiffre*  | ["a-fA-F"]*)))) as texte     { (FLOTTANT (float_of_string texte)) }
