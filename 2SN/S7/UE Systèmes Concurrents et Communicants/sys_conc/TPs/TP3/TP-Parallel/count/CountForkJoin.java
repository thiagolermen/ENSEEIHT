    ...
    static class PartialCount extends RecursiveTask<Integer> {
        ...
        public Integer compute() {
            if ((this.end - this.start) > this.threshold){
                PartialCount sp1 = new PartialCount(this.array, this.start, this.start + (this.end - this.start)/2, this.threshold);
                PartialCount sp2 = new PartialCount(this.array, this.start + (this.end - this.start)/2 , this.array.length, this.threshold);
                
                sp1.fork();
                sp2.fork();

                return sp1.join() + sp2.join();
            }else{
