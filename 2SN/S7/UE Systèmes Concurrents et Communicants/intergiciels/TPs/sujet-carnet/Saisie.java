...
public class Saisie extends JApplet {
	...
	class CButtonAction implements ActionListener {
		public void actionPerformed(ActionEvent ae) {
			...
			int numCarnet = Integer.parseInt(c.substring(c.length() - 1));
			try {
				Carnet carnet = new CarnetImpl(numCarnet);
				carnet = (Carnet) Naming.lookup("//localhost:4000/Carnet" + numCarnet);
				RFiche user = carnet.Consulter(n, true);
				message.setText("User name: " + user.getNom() + " User email: " + user.getEmail());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	class AButtonAction implements ActionListener {
		public void actionPerformed(ActionEvent ae) {
			...
			int numCarnet = Integer.parseInt(c.substring(c.length() - 1));
			try {
				SFiche user = new SFicheImpl(n, e);
				Carnet carnet = new CarnetImpl(numCarnet);
				Naming.rebind("//localhost:4000/Carnet" + numCarnet, carnet);
				carnet.Ajouter(user);
			} catch (Exception exc) {
				exc.printStackTrace();
			}
