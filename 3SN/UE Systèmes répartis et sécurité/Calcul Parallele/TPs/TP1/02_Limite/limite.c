...
// Answers to questions:
  ...
  // (a) For small messages, MPI_Send is likely to behave asynchronously, 
  // while for large messages, it tends to behave synchronously.
  // (b) If the message size is such that MPI_Send becomes synchronous, 
  // the program may experience performance issues and longer execution times.
  // (c) Estimating the limit is complex and depends on various factors, 
  // but let's assume a limit of around 10 integers.
  // (d) To overcome the limit, you can consider using 
  // MPI_Isend/MPI_Irecv for asynchronous communication, 
  // or MPI_Sendrecv for synchronous communication of larger data.
