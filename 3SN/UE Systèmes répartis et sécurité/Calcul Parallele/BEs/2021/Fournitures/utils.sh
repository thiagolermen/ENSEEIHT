...
update_hostfiles() {
  echo Retrieving machinefiles for rooms
  rooms=Salles.lst
  rm $rooms
  wget http://ganesh.enseeiht.fr/firstboot/$rooms
  while read line; do
    num=$(echo $line | sed "s/c//g")
    echo $num
    echo "#$num" > hostfiles/hostfile_$num
    while read host; do
      echo $host max-slots=4 slots=4 >> hostfiles/hostfile_$num
    done < <(grep $num $rooms | cut -d":" -f2)
  done < <(cat $rooms | cut -d":" -f1 | sort -u)
}
