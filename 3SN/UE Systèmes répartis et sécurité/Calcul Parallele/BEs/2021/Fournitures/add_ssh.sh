...
if [ ! -f ~/.ssh/id_rsa_be ]; then
  ssh-keygen -t rsa -f ~/.ssh/id_rsa_be
  #cat ~/.ssh/id_rsa_be >> ~/.ssh/authorized_keys
  echo "IdentityFile ~/.ssh/id_rsa_be" > ~/.ssh/config
  chmod 700 ~/.ssh
...
while read line; do
  ...
  ssh-copy-id -f -i ~/.ssh/id_rsa_be.pub $host &> /dev/null
#  ssh-copy-id -f -i ~/.ssh/id_rsa_be.pub $name &> /dev/null
