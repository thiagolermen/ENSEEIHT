let config_filename = ref "general.cfg"
...
let read_param () =
  let typefun    = int_of_string (find_val "typefun") in
  let dim    = int_of_string (find_val "dim") in
  let typecross    = int_of_string (find_val "typecross") in
  let typemut    = float_of_string (find_val "typemut") in
  let typeconst    = int_of_string (find_val "typeconst") in
  typefun,dim,typecross,typemut,typeconst
