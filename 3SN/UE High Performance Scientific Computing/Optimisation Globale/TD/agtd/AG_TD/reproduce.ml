...
let recomputeFit v =
  ...
  l:= List.sort (fun i j -> if !(v.(i).s_fit) > !(v.(j).s_fit) then -1 else 1) !l;
  let compt=ref 0 in
...
let computeTab3 v1 v2 rem= 
  computeTab4 v1 v2 (Array.of_list (List.sort (fun x y -> if x<y then -1 else 1) (list_rfloat rem))) rem 
...
let makeTab v1 v2 prot = 
  ...
  newprot:=(List.sort (fun i j -> if pop2.(i).r_fit>pop2.(j).r_fit then -1 else 1) 
  !newprot);
  (!newprot,pop2);;
